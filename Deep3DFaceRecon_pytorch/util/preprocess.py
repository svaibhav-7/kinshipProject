...
RESAMPLING_METHOD = 3  # BICUBIC constant value for both old and new PIL versions
...
def estimate_norm(lm_68p, H):
    ...
    M = tform.params
    # Check if M is None or singular
    if M is None or not isinstance(M, np.ndarray) or M.shape != (3, 3):
        M = np.eye(3)
    elif abs(np.linalg.det(M)) < 1e-10:  # Check for near-singular matrix
        M = np.eye(3)
